...
The [Roadmap](/reference/ubiq/#roadmap) changes freely based on what the team learns.
...
- A Story has a title, a [value statement](/reference/ubiq/#value-statement), and a priority.
...
**No [acceptance criteria](/reference/ubiq/#acceptance-criteria) on Stories by default.** A Story can evolve into a [Use Case](/reference/ubiq/#use-case) (full structured spec) or stay lightweight with criteria + [Tasks](/reference/ubiq/#task) + [Tests](/reference/ubiq/#test). The team adds fidelity based on need. For trivial work, write lightweight acceptance notes on the Story. The rule: acceptance criteria must exist *somewhere* before building. The Use Case is the highest-fidelity form. The Story is the fallback for trivial work.
...
Each Use Case operates within exactly one [Aggregate](/reference/ubiq/#aggregate) boundary.

See [Appendix C](/guide/use-cases-aggregate-boundaries/) for the full Use Case structure template and a condensed example.

### Use Case and Aggregate Boundaries

One Use Case maps to one [Aggregate](/reference/ubiq/#aggregate). If the scenario crosses aggregate boundaries, create multiple Use Cases. This signals the Story can be too broad. The [application layer](/reference/ubiq/#application-layer) coordinates between them.
...
| Story does not need full UC fidelity | Criteria + [Tasks](/reference/ubiq/#task) + [Tests](/reference/ubiq/#test) on the Story. No formal Use Case template. |
...
|---------|---------|--------|
| Technical unknown | Short fixed [timebox](/reference/ubiq/#timebox) | Decision record, [prototype](/reference/ubiq/#prototype), or rejected option |
| Domain unknown | Short fixed timebox | Clarified flow or [Process/Design Event Storming](/reference/ubiq/#process-design-event-storming) |
| Architectural unknown | Short fixed timebox | [ADR](/reference/ubiq/#adr-architecture-decision-record) (Architecture Decision Record) |

Studies do not produce production code. If a Study exceeds its [timebox](/reference/ubiq/#timebox), the team decides: rescope or run a [Go/No-Go](/reference/ubiq/#go-no-go-gate) on the parent Feature.
...
Record [Chores](/reference/ubiq/#chore) alongside Tasks under their parent Story or Use Case. If chore volume is high, use a dedicated Chore template. Do not track Chores as formal Issues unless the team finds it helpful.
...
Each Task has: [Inputs](/reference/ubiq/#inputs), [Outputs](/reference/ubiq/#outputs), [Constraints](/reference/ubiq/#constraints), [Acceptance Criteria](/reference/ubiq/#acceptance-criteria), and a [Definition of Done](/reference/ubiq/#definition-of-done).

See [Appendix G](/guide/task-structure-reference/) for the full Task structure template.
...
**Characteristics:**
- One Test per [Gherkin](/reference/ubiq/#gherkin) scenario, the main success path and each extension
- Technology-agnostic: describes the behaviour to verify, not the test framework
- Versioned alongside code

Each Test has: ID, [Source](/reference/ubiq/#source-test), [Scenario](/reference/ubiq/#scenario), Type, [Preconditions](/reference/ubiq/#preconditions), and [Expected Result](/reference/ubiq/#expected-result).

See [Appendix H](/guide/test-structure-reference/) for the full Test structure template.

**How Tests relate to other work items:**
- Use Cases produce Gherkin scenarios ([Build Phase Step 2](/guide/delivery/#build-phase)) → each scenario becomes one Test
- Stories that skip full Use Case structure use lightweight acceptance notes on the Story, not structured [Tests](/reference/ubiq/#test)
- Tests are not Tasks. They verify. Tasks build
- Unit tests are not captured as Work Items. They are part of the codebase, implied by [quality gates](/reference/ubiq/#quality-gates)
