...
Every team defines a [Team API](/reference/ubiq/#team-api): services, [SLOs](/reference/ubiq/#slo-service-level-objective), communication channels, change notifications. Without a clear API, teams step on each other.

## Human Teams

A human team is stream-aligned and cross-functional. The team is small enough to hold its work in collective working memory. Anyone can talk to a [stakeholder](/reference/ubiq/#stakeholder) or user directly. No [gatekeepers](/reference/ubiq/#gatekeeper).

### Key Characteristics

- **Stream-aligned.** The team owns a complete [value stream](/reference/ubiq/#value-stream) from start to end. No handoffs.
...
- **Psychological safety.** Respect is a performance multiplier. Blameless [retros](/reference/ubiq/#retrospective-retro). Sustainable pace. Collective code ownership. When an incident happens, the team asks "what caused this?" not "who caused it?"
...
- Same [values](/reference/ubiq/#value): Communication, Simplicity, Feedback, Courage, Respect. Same [practices](/reference/ubiq/#practice): TDD, CI, pair programming, retrospectives, [delivery cycles](/reference/ubiq/#delivery-cycle).
- Same [work items](/reference/ubiq/#work-item): Features, Stories, [Use Cases](/reference/ubiq/#use-case), Studies, Tasks, Tests.
- Same [quality gates](/reference/ubiq/#quality-gates): unit tests, integration tests, SAST, SCA.

### What Changes

- Roles become specialised agents with [system prompts](/reference/ubiq/#system-prompt) and input/output contracts.
- Communication flows through an [Orchestrator](/reference/ubiq/#orchestrator). Hub-and-spoke model. Agents do not talk to each other directly, unless specifically designed to do so.
...
A role is a responsibility, not an individual. One individual can hold multiple roles. One [role](/reference/ubiq/#role) can be fulfilled by a person or an agent. The guide describes what each role does. The team decides who fills it.
...
| **Software Engineer** | Writes clean, testable code. Uses TDD and [pair programming](/reference/ubiq/#pair-programming). Integrates often. |
| **QA Engineer** | Builds quality in from the start. Automated testing. Acceptance criteria verification. |
| **Release Owner** | Owns release decisions. Manages [feature flags](/reference/ubiq/#feature-flag). Quality gates. Rollback plans. |
| **Data Analyst** | Turns data into decisions. DORA metrics. Flow metrics. [Monte Carlo](/reference/ubiq/#monte-carlo) forecasting. |
...
| **Site Reliability Engineer** | Production reliability. Monitoring. Incident response. [Error budgets](/reference/ubiq/#error-budget). |
| **Security Champion** | Embedded security guidance within the team. Guides practices, not gates. |
| **FinOps Engineer** | Financial accountability. Cloud cost optimisation. [Agent cost](/reference/ubiq/#agent-cost) monitoring. |
